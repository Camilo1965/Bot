...
GET /api/journal/vibe-insights   — latest VIBE behavioral analysis artifact
GET /api/journal/vibe-patterns   — latest VIBE pattern detection per symbol
"""
...
_VIBE_JOURNAL_FILE = _REPO / "logs" / "vibe_journal_analysis.json"
_VIBE_PATTERNS_FILE = _REPO / "logs" / "vibe_patterns_latest.json"
...
@router.get("/api/journal/vibe-insights")
async def get_vibe_insights() -> dict:
    """Latest VIBE journal analysis (behavioral biases). Auto-generated by bot."""
    data = _read_json_file(_VIBE_JOURNAL_FILE)
    if not data:
        return {"available": False, "generated_at": None, "analysis": None}
    return {
        "available": True,
...
@router.get("/api/journal/vibe-patterns")
async def get_vibe_patterns() -> dict:
    """Latest VIBE pattern detection per symbol. Auto-generated every 15min."""
    data = _read_json_file(_VIBE_PATTERNS_FILE)
    if not data:
        return {"available": False, "generated_at": None, "patterns": {}}
    return {
        "available": True,
        "generated_at": data.get("generated_at"),
