...
if exist "logs\cloudflared.pid" (
    ...
    powershell -NoProfile -Command ^
        "$p=Get-Process -Id !CFPID! -EA SilentlyContinue; if($p){$p.Kill();Write-Host 'Tunel cerrado.'}else{Write-Host 'Tunel ya cerrado.'}"
...
echo.
echo Todo detenido.
echo Para exportar logs: doble clic en EXPORT_DAY_REVIEW.bat
echo.
pause
endlocal
