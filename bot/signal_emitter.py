...
# Default "0" (disabled) — enable via VIBE_MCP_ENABLED=1 in .env when the
# local MCP server is running.  Previous builds had this block duplicated 3×
# with the last copy overriding ENABLED to "0" silently; now there is one
...
# Keeps strong references to fire-and-forget background tasks so the GC cannot
# silently drop them before they complete (see Python docs on asyncio.create_task).
    ...
    try:
        while True:
            ...
            for symbol in watchlist:
                ...
                # [VIBE MCP SERVER] — fire-and-forget with strong task reference
                if _VIBE_MCP_ENABLED:
                    ...
                    # Fire-and-forget — keep a strong reference so GC cannot drop it.
